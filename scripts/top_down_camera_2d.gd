...
@export_range(0.01, 1, 0.01) var max_zoom_level : float = 5.0
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed(zoom_in_input):
		...
		target_zoom = target_zoom.clamp(Vector2.ONE * min_zoom_level, Vector2.ONE * max_zoom_level)  # Add this!
	...
	if Input.is_action_just_pressed(pan_input):
		# Initialize prev_mouse_pos when starting to pan
		prev_mouse_pos = curr_mouse_pos
	elif Input.is_action_pressed(pan_input):
		# Calculate the mouse movement vector
		var mouse_delta = prev_mouse_pos - curr_mouse_pos
		
		# Rotate the mouse movement by the camera's rotation
		var rotated_delta = mouse_delta.rotated(camera.rotation)
		
		target_position += rotated_delta
...
func move_to_position(target_pos: Vector2, instant: bool = false) -> void:
	"""
	Moves the camera to the specified position.
	
	Args:
		target_pos: The world position to move the camera to
		instant: If true, moves instantly. If false, uses smooth interpolation
	"""
	if use_boundaries:
	...
	if instant:
		...
		# Move smoothly using the existing interpolation system
		target_position = target_pos

func change_default_zoom(max_level: float, min_level: float):
	max_zoom_level = max_level
	min_zoom_level = min_level
