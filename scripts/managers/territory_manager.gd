...
func place_factory_rpc(placing_player_id: int, factory_type: String, hex_pos: Vector2):
	...
	var Ground = tree.current_scene.get_node("Map/Ground") as TileMapLayer
	var BuildingsParent = tree.current_scene.get_node("Buildings")
	var factory = null
	var global_hex_pos = Ground.map_to_local(hex_pos)
	...
	BuildingsParent.add_child(factory)
